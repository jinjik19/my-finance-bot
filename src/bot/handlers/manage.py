...
def get_main_manage_keyboard():
    ...
    builder.row(
        InlineKeyboardButton(text="🔔 Напоминания", callback_data="manage_scheduler")
    )
...
async def to_envelopes_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        "Меню управления конвертами:",
        reply_markup=get_envelopes_manage_menu()
    )
...
async def to_categories_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        reply_markup=get_categories_manage_menu()
    )
...
async def to_goals_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        reply_markup=get_goals_manage_menu()
    )
...
async def to_phases_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        "Меню управления Финансовыми Фазами:",
        reply_markup=get_phases_manage_menu()
    )
...
async def to_scheduler_menu(callback: CallbackQuery):
    await callback.message.edit_text(
        "Меню управления напоминаниями и задачами:",
        reply_markup=get_scheduler_manage_menu()
    )
