...
-- ====================================================================
-- 8. Сравнение личных трат
-- Тип визуализации: Гистограмма (с накоплением)
-- Настройки: Ось X - Категория, Ось Y - Сумма, Разбить по - Имя пользователя
-- ====================================================================
WITH filtered_transactions AS (
  SELECT *
  FROM transactions
...
WHERE 
    WHERE 
    (e.name = '👨 Личные (Женя)' OR e.name = '👩‍🦱 Личные (Таня)')
GROUP BY c.name, u.username;


-- ====================================================================
-- 9. Список постоянных платежей
-- Тип визуализации: Таблица
-- Что показывает: Список трат из конверта "Постоянные расходы".
-- ====================================================================
WITH filtered_transactions AS (
  SELECT *
  FROM transactions
